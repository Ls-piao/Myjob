			...
			<view v-if="disdingwei" id="hot">
				...
				<view class="dingwei">
					...
					<view class="dingwei_city">
						<view class="dingwei_city_one" @click="getWarpweft(0)">{{ position }}</view>
						<view class="dingweis_div" @click="getWarpweft(0)">
				...
				<view class="dingwei" v-if="Visit.length >= 0">
					...
					<view class="dingwei_city dingwei_city_zuijin">
						<view class="dingwei_city_one toright" v-for="(item, index) in Visit" v-if="index < 2" @click="back_city(item)" :key="index">{{ item.cityName }}</view>
					</view>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		getWarpweft(type) {
			...
			if(type)
			{
				let StorageRegion = uni.getStorageSync("region");
				if(StorageRegion)
				{
					that.$emit('region_city', StorageRegion);
					return false;
				}
			}
			...
			let countdown = setInterval(() => {
				...
				uni.getLocation({
					...
					success: function(res) {
						...
						//发送请求查询地址  https://restapi.amap.com/v3/geocode/regeo?parameters
						...
						uni.request({
							...
							success: res => {
								if (!res) {
									that.$emit('region_city', '地址解析失败');
									return false;
								}

								let region = res.data.regeocode.addressComponent.district;
								that.position = region;
								that.$emit('region_city', region);
