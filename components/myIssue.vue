	...
	<view class='issue'>
		<view class="issue-head">
			...
			<view class="issue-head-star-box" v-if="starsShow">
				<image v-for="(item,index) in starsMax" :key="index" :src="(index+1)>fscore?starDefault:starActive" :class="fscore==index+1?'active':''" mode="" @click="setScore(index+1)"></image>
			</view>
		</view>
		 <textarea v-if="textareaShow" @blur="blur" v-model="infoReceive.textareaValue" :placeholder="textareaPlaceholder"/>
